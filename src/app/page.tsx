import SearchBar from "@/components/SearchBar";
import MedicineCard from "@/components/MedicineCard";
import Link from "next/link";
import type { Metadata } from "next";
import { siteUrl } from "@/lib/site-url";
import { getTopDeals } from "@/lib/medicines-dal";

export const revalidate = 3600;

export const metadata: Metadata = {
  title: "GeneriQ — Compare UK Medicine Prices & Find Cheaper Generics",
  description: "Compare medicine prices across UK pharmacies. Find cheaper generics, check NHS vs retail costs, and get price drop alerts. Save on prescriptions instantly.",
  alternates: { canonical: siteUrl },
  openGraph: {
    title: "GeneriQ — Compare UK Medicine Prices & Find Cheaper Generics",
    description: "Compare medicine prices across UK pharmacies. Find cheaper generics, check NHS vs retail costs, and get price drop alerts.",
    url: siteUrl,
    siteName: "GeneriQ",
    type: "website",
  },
};

const PRIMARY_CATEGORIES = [
  {
    href:     "/search?tab=medicines",
    label:    "Medicines",
    count:    "945+",
    desc:     "Compare NHS-licensed drugs, Drug Tariff prices & generic alternatives",
    gradient: "from-blue-50 to-blue-100/50 dark:from-blue-950/40 dark:to-blue-900/20",
    border:   "border-blue-200 hover:border-blue-400 dark:border-blue-800 dark:hover:border-blue-600",
    iconBg:   "bg-blue-600",
    icon: "M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z",
  },
  {
    href:     "/search?tab=supplements",
    label:    "Supplements",
    count:    "5,994+",
    desc:     "Vitamins, minerals, protein & more",
    gradient: "from-amber-50 to-amber-100/50 dark:from-amber-950/40 dark:to-amber-900/20",
    border:   "border-amber-100 hover:border-amber-300 dark:border-amber-900 dark:hover:border-amber-700",
    iconBg:   "bg-amber-500",
    icon: "M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z",
  },
  {
    href:     "/search?tab=skincare",
    label:    "Skincare & Beauty",
    count:    "5,282+",
    desc:     "Serums, moisturisers, SPF & more",
    gradient: "from-pink-50 to-pink-100/50 dark:from-pink-950/40 dark:to-pink-900/20",
    border:   "border-pink-100 hover:border-pink-300 dark:border-pink-900 dark:hover:border-pink-700",
    iconBg:   "bg-pink-500",
    icon: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z",
  },
];

const MORE_CATEGORIES = [
  { href: "/search?tab=haircare",    label: "Hair Care" },
  { href: "/search?tab=dental",      label: "Dental" },
  { href: "/search?tab=baby",        label: "Baby" },
  { href: "/search?tab=pet",         label: "Pet Care" },
  { href: "/search?tab=sports",      label: "Sports Nutrition" },
];

const VALUE_PROPS = [
  {
    title: "Cheaper generics",
    desc:  "Same active ingredient, fraction of the price. We flag every cheaper alternative.",
    icon:  "M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4",
  },
  {
    title: "NHS vs retail",
    desc:  "See if buying over the counter beats your prescription charge — or vice versa.",
    icon:  "M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h.01M12 11h.01M15 11h.01M4 19h16a2 2 0 002-2V7a2 2 0 00-2-2H4a2 2 0 00-2 2v10a2 2 0 002 2z",
  },
  {
    title: "Price drop alerts",
    desc:  "Set an alert on any product and we\u2019ll email you when the price falls.",
    icon:  "M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9",
  },
];

const HOW_IT_WORKS = [
  {
    num: "1",
    title: "Search a medicine",
    desc:  "By name, brand, or active ingredient.",
    icon:  "M21 21l-4.35-4.35M17 11A6 6 0 1 1 5 11a6 6 0 0 1 12 0z",
  },
  {
    num: "2",
    title: "Compare UK prices",
    desc:  "Boots, Superdrug, H&B, Amazon \u2014 sorted cheapest first.",
    icon:  "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z",
  },
  {
    num: "3",
    title: "Save money",
    desc:  "Buy at the cheapest price, or check if your NHS prescription is cheaper.",
    icon:  "M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z",
  },
];

export default async function HomePage() {
  const deals = await getTopDeals(6).catch(() => []);

  return (
    <div className="min-h-screen">

      {/* ── Hero ───────────────────────────────────────────────────────────── */}
      <section className="border-b border-[var(--color-border)]">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-14 sm:py-20 text-center">

          {/* Badge */}
          <div className="inline-flex items-center gap-2 bg-[var(--color-brand-light)] text-[var(--color-brand)] text-xs font-semibold px-3 py-1.5 rounded-full mb-6">
            <svg className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" />
            </svg>
            945+ medicines with real NHS prices
          </div>

          <h1 className="text-3xl sm:text-5xl font-bold text-[var(--color-foreground)] leading-tight mb-4">
            Compare UK medicine prices.<br />
            <span className="text-[var(--color-brand)]">Save every time.</span>
          </h1>

          <p className="text-[var(--color-muted)] text-base sm:text-lg mb-8 max-w-lg mx-auto">
            Find cheaper generics, compare NHS prescription costs vs retail, and get price drop alerts &mdash; across Boots, Superdrug, Holland &amp; Barrett and more.
          </p>

          <SearchBar autoFocus size="lg" />

          <p className="mt-4 text-xs text-[var(--color-muted)]">
            Try: ibuprofen, paracetamol, sertraline, omeprazole, amoxicillin
          </p>
        </div>
      </section>

      {/* ── Value props ────────────────────────────────────────────────────── */}
      <section className="border-b border-[var(--color-border)] bg-[var(--color-surface-2)]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
            {VALUE_PROPS.map((vp) => (
              <div key={vp.title} className="flex sm:flex-col items-start sm:items-center gap-4 sm:gap-0 sm:text-center">
                <div className="w-10 h-10 rounded-full bg-[var(--color-brand)] flex items-center justify-center shrink-0 sm:mb-4">
                  <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={vp.icon} />
                  </svg>
                </div>
                <div>
                  <h3 className="font-semibold text-[var(--color-foreground)] text-sm mb-1">{vp.title}</h3>
                  <p className="text-xs text-[var(--color-muted)]">{vp.desc}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── Primary categories ─────────────────────────────────────────────── */}
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
        <h2 className="text-center text-xs font-semibold text-[var(--color-muted)] uppercase tracking-widest mb-6">
          What we compare
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {PRIMARY_CATEGORIES.map((cat) => (
            <Link
              key={cat.href}
              href={cat.href}
              className={`group flex flex-col items-center text-center p-5 sm:p-6 rounded-2xl border-2 bg-gradient-to-br transition-all ${cat.gradient} ${cat.border}`}
            >
              <div className={`w-12 h-12 rounded-xl ${cat.iconBg} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-sm`}>
                <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d={cat.icon} />
                </svg>
              </div>
              <h3 className="font-semibold text-sm text-[var(--color-foreground)] mb-0.5 leading-tight">{cat.label}</h3>
              <span className="text-xs text-[var(--color-brand)] font-medium mb-1">{cat.count} products</span>
              <p className="text-xs text-[var(--color-muted)] leading-snug">{cat.desc}</p>
            </Link>
          ))}
        </div>

        {/* Secondary categories — compact link row */}
        <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
          <span className="text-xs text-[var(--color-muted)]">Also compare:</span>
          {MORE_CATEGORIES.map((cat, i) => (
            <span key={cat.href}>
              <Link
                href={cat.href}
                className="text-xs font-medium text-[var(--color-brand)] hover:underline underline-offset-2"
              >
                {cat.label}
              </Link>
              {i < MORE_CATEGORIES.length - 1 && (
                <span className="text-[var(--color-muted)] mx-1">&middot;</span>
              )}
            </span>
          ))}
        </div>
      </section>

      {/* ── Live deals ─────────────────────────────────────────────────────── */}
      {deals.length > 0 && (
        <section className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-bold text-lg text-[var(--color-foreground)]">Compare prices right now</h2>
            <Link href="/search" className="text-sm text-[var(--color-brand)] font-semibold hover:underline underline-offset-2">
              Browse all &rarr;
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {deals.map((m) => (
              <MedicineCard key={m.id} medicine={m} />
            ))}
          </div>
        </section>
      )}

      {/* ── How it works ───────────────────────────────────────────────────── */}
      <section className="border-t border-[var(--color-border)] bg-[var(--color-surface-2)]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
          <h2 className="text-center font-bold text-lg text-[var(--color-foreground)] mb-10">How GeneriQ works</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
            {HOW_IT_WORKS.map((step) => (
              <div key={step.num} className="flex sm:flex-col items-start sm:items-center gap-4 sm:gap-0 sm:text-center">
                <div className="w-10 h-10 rounded-full bg-[var(--color-brand)] flex items-center justify-center shrink-0 sm:mb-4">
                  <svg className="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d={step.icon} />
                  </svg>
                </div>
                <div>
                  <h3 className="font-semibold text-[var(--color-foreground)] text-sm mb-1">{step.title}</h3>
                  <p className="text-xs text-[var(--color-muted)]">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── Trust strip ────────────────────────────────────────────────────── */}
      <section className="border-t border-[var(--color-border)]">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 flex flex-wrap justify-center gap-x-8 gap-y-3 text-xs text-[var(--color-muted)]">
          {[
            { icon: "M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z", label: "MHRA verified data" },
            { icon: "M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z", label: "NHS Drug Tariff prices" },
            { icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15", label: "Updated daily" },
            { icon: "M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z", label: "No account needed" },
          ].map(({ icon, label }) => (
            <div key={label} className="flex items-center gap-1.5">
              <svg className="w-4 h-4 text-[var(--color-brand)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d={icon} />
              </svg>
              {label}
            </div>
          ))}
        </div>
      </section>

    </div>
  );
}
